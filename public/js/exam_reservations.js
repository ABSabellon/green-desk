...
function refreshReservations(reservations) {
	...
	for (var i = 0; i < reservations.length; i++) {
		...
		var toAppend = '<tr data-id = '+reservations[i].id+' class = "resrows"><td class = "ressubj">' +reservations[i].exam.subject+ '</td><td class = "ressect">' +reservations[i].exam.section+ '</td><td class = "resname" data-status = "' +reservations[i].reservee.professor_status+ '" data-college = "' +reservations[i].reservee.professor_college+ '" data-base = "' +reservations[i].reservee.professor_base+ '">' +reserveeName+ '</td>';
...
function searchReservations() {
	$.ajax({
		...
		data: {term:searchResTerm, filter:filter, type:'Exam'}
	})
	.done( function(msg) {
		refreshReservations(msg.reservations);
	});
